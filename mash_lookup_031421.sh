...
do
 $MASH_PATH sketch -p 64 -o global_sketch_${i} "${local_path_background}"/"${dir_name}""${i}"/* ############&> /dev/null
...
wildcard_path=(${local_path_foreground}/*)



#FOR EACH FILE IN local_path_foreground
for file in ${wildcard_path[@]}
do
